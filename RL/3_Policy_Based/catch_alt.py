...
class Catch():
    ...
    def __init__(self, rows: int = 4, columns: int = 4, speed: float = 1.0, 
    ...
    def step(self,a):
        ...
        for ball in self.balls_xy: 
            ...
            if self.wind_con:
                if random.randint(0,10)>7 and ball[1]!=0:
                    wind_direction = random.randint(-1, 1)
                    if ball[0]==self.columns-1 and wind_direction==1:
                        ball[0] = ball[0]
                    elif ball[0]==0 and wind_direction==-1:
                        ball[0] = ball[0]
                    else:
                        ball[0] += wind_direction # a constant wind blow the ball to the left
                else:
                    ball[0] = ball[0]
