...
class NstepQLearningAgent:
	...
	def update(self, states, actions, rewards, done):
		...
		T_ep = len(actions) #maximum episode length

		for t in range(T_ep):
			m = min(self.n, T_ep-t) #m won't always be able to go n steps ahead

			Gt = 0
			for i in range(m):
				if done and (t+m == T_ep): #can finish loop without reaching terminal state
					Gt += self.gamma**i * rewards[t+i]
				else:
					Gt += self.gamma**i * rewards[t+i] + (self.gamma**m * np.max(self.Q_sa[states[t+m]]))
...
def n_step_Q(n_timesteps, max_episode_length, learning_rate, gamma, 
	...
	# for i in range(n_timesteps):
		...
		# if plot:
		   # env.render(Q_sa=pi.Q_sa,plot_optimal_policy=True,step_pause=0.2) # Plot the Q-value estimates during n-step Q-learning execution
